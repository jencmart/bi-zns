...
public class UserInterface {
    ...
    public Double askUser(String question) throws ActionException {
        System.out.println("Zadej hodnotu " + question + ":");
    ...
    private void explainRecursively(Rule current,int tabSpaces, List<Rule> allUsedRules, Map<String, Double> userAnswers) {
        ...
        System.out.println(current.variable.getName().replace('_', ' ') + " BECAUSE: " + current.toString());

        for(VarAndVal varAndVal : current.formula.getVariableList()) {
            String name = varAndVal.getVariable().getName();

            if(userAnswers.containsKey(name)) {
                for(int i = 0 ; i < tabSpaces+1; i++)
                    System.out.print("\t");

                System.out.println(name.replace('_', ' ') + " = " + varAndVal.getValue() + " BECAUSE: " + " Uzivatel odpovedel: Zadej hodnotu " + name.replace('_', ' ') + " => odpoved: " + userAnswers.get(name) + " => [defuzz:" + varAndVal.getVariable().getDefuzzValue() + "] := {" + varAndVal.getVariable().getSets() + "}");
            } else {
